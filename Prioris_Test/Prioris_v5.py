...
def update_suggested_tasks():
    """Fetch and display suggested tasks based on current Wi-Fi, scrollable if >4."""
    wifi = get_local_ip()
    wifi_tags = []
    ...
    def create_task_pill(text):
        ...
        canvas_pill.create_polygon(
            ...
            smooth=True,
            fill="#BFD3FF",  # soft translucent tone
            outline="#BFD3FF"
        )
    ...
    # Fetch and display all tasks matching Wi-Fi tags
    for j in wifi_tags:
    ...
    if not found_any:
        tk.Label(
            scrollable_frame,
            text="No suggested tasks for this Wi-Fi",
...
def monitor_wifi():
    ...
    wifi = get_local_ip()
    update_suggested_tasks()  # refresh once

    # schedule next refresh after 10 seconds (10,000 ms)
...
# Create a canvas inside the blue sidebar
...
# Custom blue scrollbar for the Suggested Tasks section
style = ttk.Style()
style.theme_use("clam")  # ensures color customization works
style.configure(
    "Blue.Vertical.TScrollbar",
    gripcount=0,
    background="#1555DC",     # thumb color (the draggable bar)
    darkcolor="#1555DC",
    lightcolor="#1555DC",
    troughcolor="#1555DC",    # track/background color
    bordercolor="#1555DC",
    arrowcolor="#1555DC"      # optional: makes arrows white if visible
)
